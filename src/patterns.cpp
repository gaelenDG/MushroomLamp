...
void Pattern2() {
  ...
  while (!buttonInterrupt) { // Stay in loop unless button is pressed again
    ...
    if (currentMillis - previousMillis >= interval) {
      ...
      for (int i = 0; i < 3; i++) {
        Center_NeoPixel_Chain.setPixelColor(i, Center_NeoPixel_Chain.Color(red, green, blue, white));
      }
...
//     for (int i = 0; i < numPixels; i++) {
//       startTime[i] = millis();
//       cycleDuration[i] = random(2000, 10000); // 2 to 10 sec per cycle
//       waitingForMin[i] = true; // Start in a state where we wait for the min brightness
...
//             int scaledBrightness = 25 + (brightness[i] * 230); // Scale from 25 to 255

//             // Blend between old and new color based on brightness
//             int red = map(scaledBrightness, 25, 255, colors[i][0], newColors[i][0]);
//             int green = map(scaledBrightness, 25, 255, colors[i][1], newColors[i][1]);
//             int blue = map(scaledBrightness, 25, 255, colors[i][2], newColors[i][2]);
